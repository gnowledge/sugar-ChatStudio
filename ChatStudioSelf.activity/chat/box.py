...
class TextBox(gtk.TextView):
    ...
    def __init__(self, color, bg_color, lang_rtl):
        ...
        self.fg_tag = self.get_buffer().create_tag("foreground_color",foreground=color.get_html())
        self._subscript_tag = self.get_buffer().create_tag('subscript', rise=-7 * pango.SCALE) # in pixels
        self._empty = True
        self.palette = None
    ...
    def add_text(self, text):
        ...
        for word in words:
            if _URL_REGEXP.match(word) is not None:
                tag = buf.create_tag(None,foreground="blue", underline=pango.UNDERLINE_SINGLE)
                tag.set_data("url", word)
                palette = _URLMenu(word)
                palette.connect('enter-notify-event',self.__palette_mouse_enter_cb)
                palette.connect('leave-notify-event',self.__palette_mouse_leave_cb)
...
class ChatBox(gtk.ScrolledWindow):
    ...
    def add_separator(self, timestamp):
        ...
        time_with_current_year = (time.localtime(time.time())[0],) +\
                time.strptime(timestamp, "%b %d %H:%M:%S:%f")[1:]

        timestamp_seconds = time.mktime(time_with_current_year)
        if timestamp_seconds > time.time():
            time_with_previous_year = (time.localtime(time.time())[0] - 1,) +time.strptime(timestamp, "%b %d %H:%M:%S:%f")[1:]
            timestamp_seconds = time.mktime(time_with_previous_year)

        message = ColorLabel(text=timestamp_to_elapsed_string(timestamp_seconds),color=style.COLOR_BUTTON_GREY)
    ...
    def add_log_timestamp(self, existing_timestamp=None):
        ...
        if existing_timestamp is not None:
            self._chat_log += '%s' % existing_timestamp
        """else:
            self._chat_log += '%s' % (datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S:%f'))
	This code is commented because it appends the timestamp value in chat_log"""

    def _add_log(self, nick, text, status_message):
        ...
        self._chat_log += '%s\t%s\t%d\t%s\n' % (datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S:%f'), nick, status_message, text)
